...
set dbName=readyloop
	...
	REM calendar
			...
			echo drop table public.custom_calendar cascade| psql -U postgres %dbName%	
			...
			echo CREATE TABLE IF NOT EXISTS public.custom_calendar AS select * from custom_calendar_Template;| psql -U postgres %dbName%
			...
			REM echo REFRESH MATERIALIZED VIEW returnsNasdaq WITH DATA;| psql -U postgres %dbName%
		...
		REM echo INSERT INTO exclusions_2013_2017 SELECT DISTINCT symbol, 'Return higher than 100%' as reason FROM returnsNasdaq WHERE ret>1.0; > command.txt
			...
			REM echo create view filtered as SELECT * FROM returnsNasdaq WHERE symbol NOT IN  (SELECT DISTINCT symbol FROM exclusions_2013_2017);| psql -U postgres %dbName%		
