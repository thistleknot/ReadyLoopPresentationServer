...
Rem create qs_fact table	
	...
	echo CREATE materialized view mv_qs_facts as SELECT DISTINCT ON (symbol, timestamp) id, timestamp, symbol, close, open, high, low FROM qs_facts order by symbol, timestamp, id desc; | psql -U postgres -h %host% %dbName%
