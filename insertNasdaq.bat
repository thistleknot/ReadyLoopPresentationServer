...
for /F "delims=," %%a in (tempNasdaqsymbols.txt) do (

		awk '{print F,$1,$2,$3,$4,$5,$6,$7,$8,$9}' FS=, OFS=, F=%%a c:\test\share\nasdaq\NS-%%a.csv > c:\test\share\nasdaq\NS-%%awSymbols.csv
			
		echo drop table if exists temp_table%%a;| psql -U postgres %dbName%

		echo create table temp_table_NS-%%a as table nasdaq_facts_template;|psql -U postgres %dbName%

		echo copy temp_table_NS-%%a from 'c:\test\share\%%awSymbols.csv' DELIMITER ',' CSV HEADER;| psql -U postgres %dbName%

		echo insert into nasdaq_facts select distinct * from temp_table_NS-%%a ON CONFLICT DO NOTHING;| psql -U postgres %dbName%

		echo drop table temp_table_NS-%%a;| psql -U postgres %dbName%			
